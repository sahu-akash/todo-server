// your-script.js
const option1 = process.env.OPTION1;
const option2 = process.env.OPTION2;
const firstKey = process.env.FIRST_KEY;
const projectid = process.env.INPUT_STRING;

console.log(`Option 1 selected: ${option1}`);
console.log(`Option 2 selected: ${option2}`);
console.log(`First Key from secrets: ${firstKey}`);
console.log(`projectid id: ${projectid}`);

// Your application logic here
// For example, you could call different functions based on the selected options
